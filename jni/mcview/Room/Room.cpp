...
void Room::init()
{
    ...
    static gl::index_t vIndices[12][3] = { 
		{0,1,3}, {1,2,3},	// floor
		{4,7,5}, {7,6,5},	// ceiling
		{0,4,1}, {4,5,1},	// left
		{2,6,3}, {6,7,3},	// right
    ...
    static gl::index_t tIndices[12][3] = {
		{0,1,3}, {1,2,3},	// floor
		{0,1,3}, {1,2,3},	// ceiling
		{0,1,3}, {1,2,3},	// left
		{0,1,3}, {1,2,3},	// right
...
//	std::cout << "texCoords size = " << texCoords.size() << std::endl;	
    ...
    mVbo = Vbo::create(GL_TRIANGLES);
    mVbo->set(Vbo::Attribute::createIndex()->setData(indices));
    mVbo->set(Vbo::Attribute::create("aPosition", 3)->setData(posCoords));
    mVbo->set(Vbo::Attribute::create("aNormal", 3)->setData(normals));
    // mVbo->set(Attribute::create("texcoord", 2)->setData(texCoords));
    mVbo->unbind();
}
...
// {
// 	gl::draw( mVbo );
//     // propipe draw vbo mesh not yet implemented!!!
//     // motioncel::MCG->draw()->draw(mVbo);
// }
